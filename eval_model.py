...
def init_model(args):
    ...
    if args.load == 0:
        ...
        ckp = (
            args.model_path
            or f"./{args.out_dir}/{modes[args.model_mode]}_{args.hidden_size}{moe_path}.pth"
        )
        ...
        transformers_model_path = args.model_path or "./MiniMind2"
        tokenizer = AutoTokenizer.from_pretrained(transformers_model_path)
        model = AutoModelForCausalLM.from_pretrained(
            transformers_model_path, trust_remote_code=True
        )
    print(
        f"MiniMind model parameter quantity : {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.2f}M(illion)"
    )
...
def main():
    ...
    parser.add_argument(
        "--load", default=0, type=int, help="0:  native torch weight ，1: transformers load "
    )
    parser.add_argument(
        "--model_mode",
        default=1,
    ...
    parser.add_argument("--model_path", type=str, help="Path to model which should be loaded")
