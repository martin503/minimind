...
st.markdown("""
    <style>
        ...
        .stButton button {
            ...
            padding: 0 !important;          /*  remove inner margin  */
            ...
            color: #666 !important;         /*  softer colors  */
        ...
        .stButton > button {
            ...
            margin: 0 2px !important;  /*  adjust the  margin  value  */
        }
...
def process_assistant_content(content):
    ...
    if '<think>' in content and '</think>' not in content:
        content = re.sub(r'<think>(.*?)$',
                         r'<details open style="font-style: italic; background: rgba(222, 222, 222, 0.5); padding: 10px; border-radius: 10px;"><summary style="font-weight:bold;"> under deduction ...</summary>\1</details>',
                         content,
                         flags=re.DOTALL)
...
st.sidebar.title(" model setting adjustment ")

# st.sidebar.text(" training data deviation ， when adding context memory \n multiple rounds of conversations （ more single-wheel ） prone to capacity degradation ")
...
model_source = st.sidebar.radio(" select the model source ", [" local model ", "API"], index=0)
...
if model_source == "API":
    ...
    selected_model = st.sidebar.selectbox('Models', list(MODEL_PATHS.keys()), index=2)  #  default selection  MiniMind2
...
st.markdown(
    ...
    '</div>'
    '<span style="color: #bbb; font-style: italic; margin-top: 6px; margin-bottom: 10px;"> the content is entirely from AI generate ， please be sure to identify it carefully <br>Content AI-generated, please discern with care</span>'
    '</div>',
    unsafe_allow_html=True
)
...
def main():
    ...
    prompt = st.chat_input(key="input", placeholder=" give  MiniMind  send a message ")
        ...
        with st.chat_message("assistant", avatar=image_url):
            ...
            if model_source == "API":
                try:
                    ...
                    history_num = st.session_state.history_chat_num + 1  # +1  to include the current user message 
                    ...
                    answer = f"API an error occurred in the call : {str(e)}"
